drop function if exists public.editorial_claim_due_batch(date);

create function public.editorial_claim_due_batch(p_today date default current_date)
returns jsonb[]
language plpgsql
security definer
...
begin
  ...
  select id into running_id from public.editorial_production_batches
    where status='running' order by started_at desc nulls last, created_at desc limit 1;
