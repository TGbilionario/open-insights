...
function Index() {
  ...
  const persistWatched = (next:number[]) => { setWatched(next); if (typeof window !== "undefined") localStorage.setItem("pxm-watched", JSON.stringify(next)); };
  const persistSaved = (next:number[]) => { setSaved(next); if (typeof window !== "undefined") localStorage.setItem("pxm-saved", JSON.stringify(next)); };
  const openVideo = (id:number) => { setSelected(id); setPage("videos"); if (!watched.includes(id)) persistWatched([...watched,id]); };
  const toggleSave = (id:number) => persistSaved(saved.includes(id) ? saved.filter(x => x !== id) : [...saved,id]);
  const move = (delta:number) => { const i = videos.findIndex(v => v.id === selected); const next = videos[(i + delta + videos.length) % videos.length]!; openVideo(next.id); };

  return (
    <div className="pxm-app">
      <header className="pxm-topbar">
        <button className="pxm-brand" onClick={() => setPage("home")} aria-label="Ir para início"><span className="pxm-brand-mark"><Sparkles size={16}/></span><span><b>POLÍTICA EM</b><strong>X MINUTOS</strong></span></button>
        <div className="pxm-search"><Search size={17}/><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Buscar candidato, tema ou acontecimento..."/>{query && <button onClick={()=>setQuery("")} aria-label="Limpar busca"><X size={15}/></button>}</div>
        <div className="pxm-top-actions"><button className="pxm-mobile-menu" onClick={()=>setMenu(!menu)}><Menu size={20}/></button><button className="pxm-profile-chip" onClick={()=>setPage("profile")}><span>TG</span><b>Meu perfil</b></button></div>
      </header>
      ...
      <div className="pxm-layout">
        <aside className="pxm-sidebar">
          ...
          <div className="pxm-sidebar-bottom"><div className="pxm-update"><i/><div><b>Atualização diária</b><span>Hoje · 04/09/2026</span></div></div><button className="pxm-nav-item" onClick={()=>setPage("admin")}><LayoutDashboard size={18}/> Administração</button></div>
        </aside>
...
function Home({progress,watched,open,query,results,setCategory,followedTopics,toggleTopic,topicNotice}:{progress:number;watched:number[];open:(id:number)=>void;query:string;results:VideoItem[];setCategory:(c:Category)=>void;followedTopics:string[];toggleTopic:(c:string)=>void;topicNotice:boolean}) {
  ...
  const dailyDateLabel = formatDate(previousDay); const weeklyDateLabel = formatDate(now);
  return <div className="pxm-home">
    <section className="pxm-hero"><div className="pxm-hero-copy"><div className="pxm-live"><i/> NOVA ATUALIZAÇÃO DISPONÍVEL</div><span className="pxm-eyebrow">SEXTA-FEIRA, 04 DE SETEMBRO</span><h1>Fique por dentro da política.<br/><em>Em X minutos.</em></h1><p>Os acontecimentos presidenciais que realmente importam, resumidos em vídeos curtos e objetivos.</p><div className="pxm-actions"><button className="pxm-primary" onClick={()=>open(videos.find(v=>!watched.includes(v.id))?.id || 1)}><Play size={16} fill="currentColor"/> Começar agora</button><button className="pxm-ghost" onClick={()=>open(weeklyShorts[0]!.id)}>Ver destaque <ChevronRight size={16}/></button></div></div><div className="pxm-hero-score"><div className="pxm-score-ring"><strong>{progress}%</strong><span>do dia</span></div><small>{remaining ? "Faltam "+remaining+" vídeos" : "Você já está atualizado."}</small></div></section>
    <section className="pxm-progress-card"><div><span>SEU DIA</span><strong>{watched.length} de {videos.length} vídeos assistidos</strong></div><div className="pxm-progress"><i style={{width:progress+"%"}}/></div><b>{remaining ? "Faltam "+remaining+" para você ficar por dentro de tudo." : "Você já está atualizado."}</b></section>
    {dailyHighlightAvailable && <DailyHighlight shorts={videos.slice(0,4)} dateLabel={dailyDateLabel} onOpenShort={open}/>} {weeklyHighlightAvailable && <WeeklyHighlight shorts={weeklyShorts} weekLabel={`DESTAQUE DA SEMANA · ${weeklyDateLabel}`} onOpenShort={(id)=>open(id)}/>} 
    <section className="pxm-section"><div className="pxm-section-head"><div><span>CURADORIA EDITORIAL</span><h2>{query ? `Resultados para "${query}"` : "Principais notícias de hoje"}</h2></div><button className="pxm-text-btn" onClick={()=>open(1)}>Ver todos <ChevronRight size={15}/></button></div>{results.length ? <div className="pxm-grid">{results.slice(0,6).map(v=><NewsCard key={v.id} video={v} watched={watched.includes(v.id)} open={()=>open(v.id)}/>)}</div> : <Empty title="Nada encontrado" text="Tente outro nome, tema ou acontecimento."/>}</section>
    <section className="pxm-section pxm-follow-section"><div className="pxm-section-head"><div><span>ACOMPANHE UM TEMA</span><h2>Receba novidades sobre o que importa para você</h2></div><span className="pxm-follow-count">{followedTopics.length} tema{followedTopics.length===1?"":"s"} acompanhado{followedTopics.length===1?"":"s"}</span></div><p className="pxm-follow-intro">Escolha temas para acompanhar. Quando novos conteúdos sobre eles forem publicados no produto, eles aparecem destacados para você.</p><div className="pxm-topic-pills">{categories.slice(1).map(c=><button key={c} className={followedTopics.includes(c)?"selected":""} onClick={()=>toggleTopic(c)}>{followedTopics.includes(c)&&<Check size={14}/>} {c}</button>)}</div>{topicNotice&&<div className="pxm-follow-toast"><Check size={15}/> Preferência salva neste dispositivo.</div>}</section>
    <section className="pxm-section"><div className="pxm-section-head"><div><span>EXPLORE</span><h2>Por categoria</h2></div><Filter size={18}/></div><div className="pxm-category-grid">{categories.slice(1).map((c,i)=><button key={c} onClick={()=>setCategory(c)}><small>{String(i+1).padStart(2,"0")}</small><b>{c}</b><ChevronRight size={16}/></button>)}</div></section>
    <section className="pxm-missed"><div className="pxm-missed-icon"><History size={19}/></div><div><span>O QUE VOCÊ PERDEU</span><h3>{remaining ? "Você ainda não viu "+remaining+" acontecimentos importantes de hoje." : "Você não perdeu nada importante hoje."}</h3><p>Seu progresso é salvo automaticamente neste dispositivo.</p></div><button className="pxm-ghost" onClick={()=>open(videos.find(v=>!watched.includes(v.id))?.id || 1)}>Continuar <ChevronRight size={15}/></button></section>
  </div>;
}
