...
import {
  ...
  Play, Search, Settings, Share2, Sparkles, User, Volume2, VolumeX, X, Database
} from "lucide-react";
...
function Nav({page,go}:{page:Page;go:(p:Page)=>void}) {
  ...
  return <nav className="pxm-nav"><>{items.map(([p,icon,label])=><button key={p} className={"pxm-nav-item "+(page===p?"active":"")} onClick={()=>go(p)}>{icon}{label}</button>)}<a key="editorial" className="pxm-nav-item" href="/editorial"><Database size={18}/>Banco de Conteúdo</a></></nav>;
}
